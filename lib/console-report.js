...
var compact    = require('es5-ext/lib/Array/prototype/compact')
  ...
  , lcount = clc.xterm(232), lline = clc.xterm(240), lcode = clc.xterm(250)
  , lchar = clc.xterm(250).bgXterm(235)
...
reportFile = function (name, report) {
	...
	isStatus = false;
	str += (name ? (lname(name) + ' ') : '') + lopts(report.options);
	if (report.length) {
		str += '\n';
		...
		report.forEach(function (data, index) {
			var line = report.srcLines[data.line - 1].replace(/\t/g, '    ')
			  , at = data.character - 1;
			str += lcount('#' + countPad.call(index + 1)) + ' ' + lerr(data.message)
				+ '\n' + lline(srcPad.call(data.line) + ' ')
				+ lcode(line.slice(0, at)) + lchar(line[at]) + lcode(line.slice(at + 1))
				+ '\n';
		});
	} else {
		str += ' ' + lok('OK') + '\n';
...
module.exports = function (report, options) {
	...
	process = function (report, name) {
		if (report.length) {
			if (!report.src) {
				return readFile(report.path, { loose: true })(function (src) {
					if (src) {
	...
	if (options.progress) {
		...
		promise = report(function (report) {
			if (isArray(report)) {
				reports.push(process(report));
			}
			return reports;
		});
	} else {
		promise = report(function (data) {
			reports = isArray(data) ? [process(data)] : mapToArray(data, process);
			reports.parsed = 0;
			reports.errors = 0;
			reports.failed = 0;
			return reports;
