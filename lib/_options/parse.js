...
module.exports = function (data, path) {
	var opts = {}, current, preset, ignore, lastPathLine, result;
	...
	data.forEach(function (line) {
		...
		if (newFileStartRe.test(line)) {
			...
			if (lastPathLine) current += ':' + nuPath;
			else current = nuPath;
			lastPathLine = true;
			return;
		}
			...
			if (match[3] === '+') {
				if (preset) {
					if (preset.hasOwnProperty('mod')) {
						...
						if (!isArray(preset)) opts[current][name] = toList(preset);
	...
	forEach(opts, function (value,  name) {
		...
		name.split(':').forEach(function (name) { result[name] = value; });
	});
	return result;
};
