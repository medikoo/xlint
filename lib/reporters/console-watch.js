...
Reporter.prototype = Object.create(Base.prototype, {
	...
	redraw: d(function () {
		var dist, columns, count, str, redrawAll, index, top, cls;

		if (!this.pending.length) {
			// No reports
			if (this.current.length) {
				// Clear screen
				top = 0, cls = repeat.call(' ', this.totalWidth);
				while (top < this.columnHeight) {
					this.log(clc.set(0, top++ + 3) + cls);
				}
			}
			this.current = [];
			return;
		...
		columns.forEach(function (cells, index) {
			...
			while ((diff > 1) && !areReportsFullyExposed(cells)) {
		...
		columns.forEach(function (column, index) {
			var top = 0, prevTop = 0
			  ...
			  , width = ((columns.length - 1) === index) ?
					(this.totalWidth - this.columnWidth * index - 1) : (this.columnWidth - 1)
			...
			while (top < this.columnHeight) {
				this.log(clc.set(left, top++ + 3) + repeat.call(' ', width));
			}
		}, this);
