...
LintDirectory.prototype = Object.create(LintFiles.prototype, {
	init: d(function () {
		...
		this.reader = readdir(this.root, {
			watch: this.watch, depth: this.depth, stream: this.stream,
		...
		this.reader.end(function (names) {
			(this.stream ? deferred.map(waiting) :
					deferred.map(names, this.prelint, this, 50))
...
lintDirectory = function (linter, path, options) {
	...
	name = options.watch ? 'isRootWatch' : 'isRoot';
